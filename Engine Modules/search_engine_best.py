...
class SearchEngine:
    ...
    def build_index_from_parquet(self, fn):
        ...
        for idx, document in enumerate(documents_list):
            ...
            if self._config.calculateDocumentVectorsWhileIndexing:
                ...
                self.document_vectorizer.generate_document_vector(parsed_document.term_doc_dictionary, doc_id,
                                                                  to_include_term_weights=False)
        ...
        if not self._config.toCalcVecsAfterQuery and not self._config.calculateDocumentVectorsWhileIndexing:
            self.document_vectorizer.generate_all_document_vectors(self._indexer.document_posting,
                                                                   self._config.toWeighVectors)
        to_disk = [self._indexer.inverted_idx, self._indexer.postingDict, self._indexer.document_posting,
                   self.document_vectorizer.doc_to_vec]
        # utils.save_obj(to_disk, 'inverted_idx')
    ...
    def load_index(self, fn):
        ...
        """
        index = self._indexer.load_index(fn)
        loaded_inverted_idx = index[0]
        loaded_posting_dict = index[1]
        loaded_document_posting = index[2]
        loaded_doc_vecs = index[3]
