class ConfigClass:
    def __init__(self):
        ...
        self._model_url = 'https://drive.google.com/file/d/1LA_VkKkxf1t37DR5qQhgomagHxL5gnic/view?usp=sharing'
        ...
        self.toWeighVectors = False
        self.calculateDocumentVectorsWhileIndexing = False
        self.toCalcVecsAfterQuery = False
        self.toWeighQuery = True

        if self.calculateDocumentVectorsWhileIndexing and self.toCalcVecsAfterQuery:
            raise Exception("Conflict between vector calculation")

        if self.calculateDocumentVectorsWhileIndexing and self.toWeighVectors:
            raise Exception("Cannot calculate vectors with weights during indexing")
